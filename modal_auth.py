...
def start_modal_token_new(profile: str | None, activate: bool = True, verify: bool = True) -> dict[str, Any]:
    ...
    job = {
        ...
        "verify": bool(verify),
        "status": "pending",
        "message": "Token link flow queued.",
        "exit_code": None,
        "created_at": _now_iso(),
        "updated_at": _now_iso(),
        "logs": [],
    ...
    with _TOKEN_LOCK:
        ...
        _persist_job_state(job)
    _append_log(job, f"Queued Modal token link flow for profile={normalized_profile}.")
...
def _run_modal_token_new(job_id: str) -> None:
    ...
    try:
        _set_job_fields(job_id, status="running", message="Modal account link flow running.")
        with _TOKEN_LOCK:
            job = _TOKEN_JOBS[job_id]
            cmd = [
                sys.executable,
        ...
        with _TOKEN_LOCK:
            ...
            if exit_code == 0:
                job["status"] = "success"
                job["message"] = "Modal account link flow completed successfully."
                job["error_detail"] = None
            else:
                job["status"] = "failed"
                job["message"] = "Modal account link flow failed."
        ...
        with _TOKEN_LOCK:
            job["status"] = "failed"
            job["message"] = "Modal account link flow crashed."
